...
import { providerEvidenceSnapshots } from '../persistence/schema.js';
...
export async function persistProviderEvidenceSnapshot(
  ...
  await input.db.insert(providerEvidenceSnapshots).values({
    ...
    usageEvidence: input.usageEvidence.map((item) => ({ ...item })),
    costEvidence: input.costEvidence.map((item) => ({ ...item })),
    isDemo: input.isDemo,
  });
