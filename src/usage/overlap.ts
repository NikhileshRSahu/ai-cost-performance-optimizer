...
function overlaps(a: UsageRecord, b: UsageRecord): boolean {
  return Date.parse(a.intervalStart) < Date.parse(b.intervalEnd) && Date.parse(b.intervalStart) < Date.parse(a.intervalEnd);
}
...
export function excludeUnreconciledOverlaps(records: readonly UsageRecord[]): OverlapResult {
  ...
  for (let i=0;i<records.length;i++) for (let j=i+1;j<records.length;j++) {
    ...
    const sameScope = a.organizationId===b.organizationId && a.provider===b.provider && a.model===b.model && a.workload===b.workload;
    ...
    if (a.granularity !== b.granularity || a.fingerprint !== b.fingerprint) {
      ...
      reasons.push(`UNRECONCILED_OVERLAP:${a.sourceLine}:${b.sourceLine}`);
    }
  }
  return Object.freeze({
    included:Object.freeze(records.filter(r=>!excluded.has(r))),
    excluded:Object.freeze(records.filter(r=>excluded.has(r))),
