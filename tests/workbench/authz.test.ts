...
import { authorize, type AuthenticatedSession } from '../../src/workbench/authz.js';
...
describe('session-derived authorization', () => {
  it('keeps viewers read-only', () => {
    ...
    expect(authorize(session, 'org-viewer', 'MARK_IMPLEMENTED').allowed).toBe(false);
  });
  ...
  it('lets operators execute workbench evidence actions but not owner administration', () => {
    ...
    expect(authorize(session, 'org-operator', 'MANAGE_MEMBERSHIP').allowed).toBe(false);
    expect(
  ...
  it('lets owners perform every action', () => {
    ...
    expect(authorize(session, 'org-owner', 'MANAGE_CREDENTIAL_REFERENCE').allowed).toBe(true);
  });
