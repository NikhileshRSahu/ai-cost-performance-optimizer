...
export default async function ImportPage({
  ...
  return (
    <div className="workflow-page">
      ...
      <section className="workflow-card upload-card">
        ...
        <form action={uploadUsageCsv} className="upload-form">
          ...
          <label className="file-drop">
            ...
            <input name="usageCsv" type="file" accept=".csv,text/csv" required />
          </label>
      ...
      {imported !== null ? (
        <section className="workflow-card import-result" aria-labelledby="import-result-title">
          <div className="section-heading">
            ...
            {imported.isDemo ? (
              <span className="demo-inline">Synthetic demo data — not a customer result.</span>
            ) : null}
          </div>

          <div className="summary-grid">
            <div><span>Accepted</span><strong>{imported.acceptedRows}</strong></div>
            <div><span>Duplicates skipped</span><strong>{imported.skippedRows}</strong></div>
            <div><span>Rejected</span><strong>{imported.rejectedRows}</strong></div>
            <div><span>Warnings</span><strong>{imported.warningCount}</strong></div>
          </div>

          <dl className="evidence-list">
            <div><dt>Evidence window</dt><dd>{imported.rangeStart ?? 'Unavailable'} → {imported.rangeEnd ?? 'Unavailable'}</dd></div>
            <div><dt>Checksum</dt><dd><code>{imported.checksum}</code></dd></div>
            <div><dt>Source</dt><dd>{imported.source}</dd></div>
          </dl>
