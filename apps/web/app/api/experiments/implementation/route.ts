...
export async function POST(request:Request){
  try{
    ...
    const result=await withRuntimeWorkspace(async({workspace,database})=>{await enforceRateLimit({pool:database.pool,organizationId:workspace.organizationId,scope:'implementation',limit:10,windowSeconds:600});return 
