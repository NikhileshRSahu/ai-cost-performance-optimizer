...
export function ResultJourney({
  ...
  current: 'Observed' | 'Finding' | 'Tested' | 'Verified';
}) {
  ...
  return (
    <section
      className="rounded-[20px] border border-white/[0.07] bg-white/[0.018] px-4 py-4 sm:px-5"
      aria-label="Optimization journey"
    >
      <div className="relative grid grid-cols-4 gap-2">
        <div
          className="absolute left-[8%] right-[8%] top-4 h-px bg-white/[0.08]"
          aria-hidden="true"
        />
        <motion.div
          ...
          return (
            ...
            >
              ...
              <p
                className={
                  reached
                    ? 'm-0 mt-2 text-[10px] font-semibold text-white/80'
                    : 'm-0 mt-2 text-[10px] font-semibold text-white/32'
                }
              >
                {step.label}
              </p>
              <p className="m-0 mt-.5 hidden text-[9px] text-white/28 sm:block">
                {step.caption}
              </p>
            </div>
          );
