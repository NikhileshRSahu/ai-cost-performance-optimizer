...
export default function DashboardApp({userName,userEmail,publicDemo=false,workspaceName='My workspace',realSummary}:{userName:string,userEmail:string,publicDemo?:boolean,workspaceName?:string,realSummary?:RealSummary}){
  ...
  const workspace=publicDemo?'Meridian — Sample workspace':workspaceName;
  ...
  return <main className="app-shell">
    ...
    <aside className="sidebar"><nav>{nav.map(([id,label])=><Link key={id} className={section===id || (section==='overview'&&id==='overview')?'active':''} href={id==='overview'?basePath:basePath+'/'+id}><span className="nav-icon">{id==='overview'?'▦':id==='opportunities'?'◇':id==='experiments'?'♜':id==='reports'?'□':id==='alerts'?'♧':id==='integrations'?'⌘':id==='team'?'♧':id==='billing'?'▭':'⚙'}</span>{label}</Link>)}</nav><div className="sync-note">Illustrative snapshot<br/>1.2M sample requests · 2 sample providers</div></aside>
...
function Team({userEmail}:{userEmail:string}){return <><div className="page-head"><div><p className="eyebrow">Authority follows risk</p><h1>Team & roles</h1><p>Investigation is separated from production approval.</p></div><button className="btn black" disabled>Sample workspace</button></div><div className="report-table"><div className="thead"><span>Member</span><span>Role</span><span>Can roll out</span><span>Status</span></div><div><strong>{userEmail}</strong><span>Approver</span><b>Yes</b><span>Active</span></div><div><strong>alex@meridian.io</strong><span>Engineer</span><b>No</b><span>Active</span></div><div><strong>finance@meridian.io</strong><span>Viewer</span><b>No</b><span>Active</span></div></div><div className="role-notes"><article><h3>Viewer</h3><p>Can read dashboards and reports.</p></article><article><h3>Analyst</h3><p>Can investigate evidence and create drafts.</p></article><article><h3>Engineer</h3><p>Can configure and run experiments.</p></article><article><h3>Approver</h3><p>Can authorize production rollout.</p></article></div></>}
function Billing(){return <><div className="page-head"><div><p className="eyebrow">Scale plan</p><h1>Billing</h1><p>Priced on spend under observation, never a percentage of claimed savings.</p></div></div><div className="billing-card"><div><span>Current plan</span><strong>Scale · $490/month</strong></div><div><span>Spend under observation</span><strong>$41,208 of $100,000</strong><div className="progress"><span style={{width:'41%'}}/></div></div><div><span>Next invoice</span><strong>Oct 1, 2026</strong></div><button className="btn outline" disabled>Sample billing</button></div></>}
function Settings({userName,userEmail}:{userName:string,userEmail:string}){return <><div className="page-head"><div><p className="eyebrow">Workspace administration</p><h1>Settings</h1><p>Security and product defaults are visible in one place.</p></div></div><div className="settings-grid"><article><h2>Profile</h2><label>Name<input defaultValue={userName}/></label><label>Email<input defaultValue={userEmail} disabled/></label><button className="btn black" disabled>Sample only</button></article><article><h2>Evidence policy</h2><label>Verification window<select defaultValue="14"><option value="7">7 days</option><option value="14">14 days</option><option value="30">30 days</option></select></label><label>Default quality guardrail<select defaultValue="1"><option value="0.5">−0.5%</option><option value="1">−1%</option><option value="2">−2%</option></select></label><button className="btn black" disabled>Sample only</button></article><article><h2>Session</h2><p>Sign out of this browser when you are finished.</p><form action={signOutAction}><button className="btn outline">Sign out</button></form></article></div></>}
