...
export function PromptCacheSavingsCalculator() {
  ...
  const result = useMemo(() => {
    ...
    const baseline = divide(multiply(rational(totalTokens), normalRate), million);
    const blendedRate = add(
      multiply(discountedRate, hitShare),
      multiply(normalRate, missShare),
    );
    const cachedCost = divide(multiply(rational(totalTokens), blendedRate), million);
    ...
    <div className="calculator-shell">
      <form className="calculator-form" onSubmit={(event) => event.preventDefault()}>
        <label>
          ...
          <input inputMode="numeric" value={requests} onChange={(event) => setRequests(event.target.value)} />
        </label>
        <label>
          <span>Cacheable input tokens per request</span>
          <input inputMode="numeric" value={cacheableTokens} onChange={(event) => setCacheableTokens(event.target.value)} />
        </label>
        <label>
          <span>Uncached input price per 1M tokens</span>
          <input inputMode="decimal" value={uncachedRate} onChange={(event) => setUncachedRate(event.target.value)} />
        </label>
        <label>
          <span>Cached input price per 1M tokens</span>
          <input inputMode="decimal" value={cachedRate} onChange={(event) => setCachedRate(event.target.value)} />
        </label>
        <label>
          <span>Cache hit rate (%)</span>
          <input inputMode="decimal" value={hitRate} onChange={(event) => setHitRate(event.target.value)} />
        </label>
        <label>
      ...
      <section className="calculator-results" aria-live="polite">
        {result === null ? (
          <p>Enter non-negative prices and a cache hit rate from 0 through 100.</p>
        ) : (
          <>
            <div className="metrics-grid">
              <article className="metric-card">
                <span className="metric-label">Potential monthly cache saving</span>
                <strong className="metric-value">
                  {currency} {formatDecimal(result.savings, 2)}
                </strong>
              </article>
              <article className="metric-card">
                <span className="metric-label">Baseline cacheable-input cost</span>
                <strong className="metric-value">
            ...
            <dl className="calculator-breakdown">
              <div>
                ...
                <dd>{currency} {formatDecimal(result.annualSavings, 2)}</dd>
              </div>
              <div>
