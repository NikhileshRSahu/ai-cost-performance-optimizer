  ...
  return (
    <section className="relative mx-auto w-full max-w-5xl overflow-hidden rounded-[28px] border border-white/[0.08] bg-[#070b10] text-white shadow-[0_38px_110px_rgba(0,0,0,.34)]">
        ...
        <div className="mt-3 flex flex-col gap-3 sm:flex-row sm:items-end sm:justify-between">
          <div>
            ...
            <p className="m-0 mt-3 max-w-2xl text-sm leading-6 text-white/45">
              Choose one source. Evalomics analyzes it automatically and takes you
              straight to the strongest supported result.
            </p>
          </div>
          <div className="inline-flex items-center gap-2 text-[11px] text-white/35">
            <ShieldCheck className="size-4 text-emerald-300/70" />
            Credentials stay scoped to usage analysis.
          </div>
        </div>
      </div>
        ...
        {sources.map((source, index) => (
          <motion.div
            ...
            transition={
              reduceMotion
                ? { duration: 0 }
                : { duration: 0.34, delay: index * 0.06, ease: [0.16, 1, 0.3, 1] }
            }
          >
            <Link
              href={source.href}
              className="group flex min-h-64 h-full flex-col rounded-[20px] border border-white/[0.08] bg-[linear-gradient(145deg,rgba(15,23,33,.94),rgba(8,12,17,.98))] p-5 no-underline transition duration-200 hover:-translate-y-1 hover:border-cyan-300/20"
            >
      ...
      <div className="relative border-t border-white/[0.06] px-6 py-4 text-[11px] leading-5 text-white/30 sm:px-8">
        Evalomics separates observed usage, potential savings, tested outcomes, and
        verified production savings. Connecting a source does not turn projections
        into proof.
      </div>
    </section>
  );
