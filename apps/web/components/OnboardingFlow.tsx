...
export default function OnboardingFlow({name,email}:{name:string,email:string}){
  ...
  const [workspace,setWorkspace]=useState('Meridian — Production');
  ...
  function finish(){
    localStorage.setItem('evalomics:onboarded','1');
    localStorage.setItem('evalomics:workspace',workspace);
  ...
  return <main className="onboard-page blueprint">
    ...
    <div className="onboard-layout">
      ...
      <section className="onboard-main">
        {step===1 && <><h1>Create your workspace account</h1><p>Signed in as <strong>{name || email}</strong>. Your authentication is live; nothing about the sample workspace touches production traffic.</p><div className="form-panel"><label>Work email<input value={email} disabled/></label><label>Name<input value={name} disabled/></label><button className="btn black full" onClick={()=>setStep(2)}>Continue to workspace</button></div></>}
        {step===2 && <><h1>Name the workspace</h1><p>Use the name your engineering and finance teams already recognize.</p><div className="form-panel"><label>Workspace name<input value={workspace} onChange={e=>setWorkspace(e.target.value)}/></label><label>Default role<select defaultValue="Approver"><option>Approver</option><option>Engineer</option><option>Analyst</option><option>Viewer</option></select></label><button className="btn black full" onClick={()=>setStep(3)}>Save workspace and connect providers</button></div></>}
        {step===3 && <><h1>Connect your providers</h1><p>Paste read-only keys. Evalomics ingests usage and cost metadata — never prompts or completions by default. Read-only means we cannot touch your traffic.</p><div className="provider-card"><div><strong>OpenAI</strong><span>Read-only usage scope</span></div><input value={openai} onChange={e=>setOpenai(e.target.value)} placeholder="sk-••••" type="password"/></div><div className="provider-card"><div><strong>Anthropic</strong><span>Read-only usage scope</span></div><input value={anthropic} onChange={e=>setAnthropic(e.target.value)} placeholder="sk-ant-••••" type="password"/></div><div className="csv-box">No keys handy? <label className="linklike">Upload a usage export (CSV)<input type="file" accept=".csv" hidden onChange={()=>{setStep(4);setSyncState('done')}}/></label> instead.</div><button className="btn black full" onClick={startSync}>Connect and start sync</button><p className="micro">For this deployment, provider credentials are not persisted by the browser demo. CSV and the sample workspace are safe ways to inspect the full flow.</p></>}
