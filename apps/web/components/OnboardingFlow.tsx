  ...
  return <main className="onboard-page blueprint">
    ...
    <div className="onboard-layout">
      ...
      <section className="onboard-main">
        ...
        {step===2 && <><h1>Name the workspace</h1><p>Use the name your engineering and finance teams already recognize.</p><div className="form-panel"><label>Workspace name<input value={workspace} onChange={e=>setWorkspace(e.target.value)} placeholder="Acme — Production"/></label><label>Default role<select defaultValue="Approver"><option>Approver</option><option>Engineer</option><option>Analyst</option><option>Viewer</option></select></label><button className="btn black full" disabled={!workspace.trim()} onClick={()=>setStep(3)}>Save workspace and connect providers</button></div></>}
