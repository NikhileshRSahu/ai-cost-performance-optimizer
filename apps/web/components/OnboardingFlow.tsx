    ...
    <div className="onboard-layout">
      ...
      <section className="onboard-main">
        ...
        {step===3&&<><h1>Connect your providers</h1><p>Use organization-level read-only/admin reporting keys so Evalomics can read usage and cost evidence. Credentials are encrypted before storage and are never shown again.</p>
          <div className="provider-card"><div><strong>OpenAI API</strong><span>API Platform organization usage + costs · not personal ChatGPT usage</span></div><input value={openai} onChange={e=>setOpenai(e.target.value)} placeholder="sk-admin-••••" type="password" autoComplete="off"/></div>
          <div className="provider-card"><div><strong>Anthropic API <small className="beta-chip">BETA</small></strong><span>Admin API usage + costs · real production verification still required</span></div><input value={anthropic} onChange={e=>setAnthropic(e.target.value)} placeholder="sk-ant-admin-••••" type="password" autoComplete="off"/></div>
